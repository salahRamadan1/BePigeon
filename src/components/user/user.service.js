...
exports.updateUser = createTryAndCatch(async (req, res, next) => {
  //  first find user
  const user = await UserModel.findById(req.body._id);
  // if user not exits
  if (!user) return next(new AppError(401, "user not found"));
  //  change user
  let IsUser = await UserModel.findByIdAndUpdate(
    req.body._id,
    {
...
// change profile pic
exports.changeProfilePic = createTryAndCatch(async (req, res, next) => {
...
// get user info
exports.getInfoUser = createTryAndCatch(async (req, res, next) => {
  let user = await UserModel.findOne(req.id);
